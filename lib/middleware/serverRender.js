...
const serverRender = (clientRoutePath, reducerPath, sagaPath) => async (ctx, next) => {
  ...
  if (process.env.NODE_ENV !== 'production' && process.env.DEPLOY_ENV !== 'pre' && process.env.DEPLOY_ENV !== 'prd' && process.env.DEPLOY_ENV !== 'test') {
  ...
  // const isNotRootRoute = ({ match }) => match.path !== '/';  --> modified by breeze 
  ...
  // const matchedRouter = matchRoutes(routes, ctx.request.path).filter(isNotRootRoute); --> modified by breeze
  const matchedRouter = matchRoutes(routes, ctx.request.path);
  ...
  const promises = matchedRouter.map(({ route }) =>
    (route.component.loadInitialData instanceof Function ?
      route.component.loadInitialData(store, ctx.request)
      :
