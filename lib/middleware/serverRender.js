...
const serverRender = (clientRoutePath, reducerPath, sagaPath) => async (ctx, next) => {
  ...
  const routes = require(`${clientRoutePath}/${ctx.__CONTAINER__ENV__.name}/router.js`); // eslint-disable-line global-require
  ...
  const renderHtmlString = async (inititalDataArr) => {
    ...
    await ctx.render(ctx.__CONTAINER__ENV__.publicPath, {
